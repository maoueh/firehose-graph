...
function main() {
  ...
  generate "sf/graph/type/v1/type.proto"

  echo "generate.sh - `date` - `whoami`" > ./last_generate.txt
  echo "streamingfast/firehose-graph/proto revision: `GIT_DIR=$ROOT/.git git log -n 1 --pretty=format:%h -- proto`" >> ./last_generate.txt
}
